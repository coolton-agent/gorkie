...
- Delegations run in the foreground when _background is omitted. Use _background.enabled=true deliberately when the work is independent, long-running, or one of several children that should run concurrently.
- After a background agent reports that it started, end the current turn immediately with no substantive response. Do not call another tool, repeat the delegated work, give a partial answer, or tell the user to wait. Mastra will return the completed result and resume you automatically.
- Never poll a background child, sleep, call wait, or launch a duplicate because it has not finished. A start acknowledgement is not a result. On resumption, use the returned child result before deciding whether any specific gap remains.
...
- Set only the delegation prompt and optional background mode. Leave instructions and maxSteps unset; the harness owns child instructions and execution budgets.
